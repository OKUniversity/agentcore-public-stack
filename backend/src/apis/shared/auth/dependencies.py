...
async def get_current_user_id(
    ...
    Convenience wrapper around `get_current_user_from_session` that returns
    just the user_id field. Cookie-only since Phase 7 removed the Bearer
    fallback for SPA-facing routes.
    """
    return user.user_id
